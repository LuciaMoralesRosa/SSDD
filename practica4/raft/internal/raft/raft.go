...
type NodoRaft struct {
	...
	Aplicada		map[int]chan string 	// Indicar a los clientes que su
...
func NuevoNodo(nodos []rpctimeout.HostPort, yo int, 
	...
	nr.Aplicada = make(map[int] chan string)
...
func (nr *NodoRaft) AppendEntries(args *ArgAppendEntries,
	...
	// Completar....
	nr.Logger.Printf("%d: He recibido un appendEntries de %d con mandato %d y" +
	" entradas %v\n", nr.Yo, args.LeaderId, args.Term, args.Entradas)
...
func (nr *NodoRaft) guardarEntradasEnLog(args *ArgAppendEntries) {
	if !esLatido(args.Entrada) {
		...
		indiceLog := args.PrevLogIndex +1 // Indice del log donde empezar a insertar entradas
		indiceEntradas := 0 // Indica a partir de donde copiar de las entradas enviadas
		...
		if indiceEntradas < len(args.Entradas) {
			nr.Logger.Printf("%d: Se van a insertar las entradas %v desde el " +
			"indice %d\n", nr.Yo, args.Entradas[indiceEntradas:], indiceLog)
			...
			nr.Logger.Printf("%d: Mi nuevo log es %v\n", nr.Log)
		}
	}
...
func (nr *RaftNodo) aplicarEntradasEnMaquinas() {
	nr.Mux.Lock()
	...
	for entrada := 0; entrada < len(entradas)-1; entradas++ {
...
func posicionEncontrada(indiceLog int, indiceEntradas int, log []EntradaLog, entradas []EntradaLog) {
	// Dentro de limites
...
func (nr *NodoRaft) protocoloSeguidor() {
	select {
		...
		case <-SoySeguidor:
			nr.Logger.Println("Nodo %d: Soy seguidor y sigo siendolo en " +
...
func (nr *NodoRaft) protocoloCandidato() {
	...
	select {
		...
		case <-nr.SoySeguidor: 
			nr.Estado = SEGUIDOR
			nr.Logger.Println("Nodo %d: He perdido la eleccion y paso a ser " +
		...
		case <- nr.SoyLider:
			nr.Estado = LIDER
			nr.Logger.Println("Nodo %d: He ganado la eleccion y paso a ser " +
				"lider en el mandato %d \n", nr.Yo, nr.CurrentTerm)
		// Vence el timeout: sigue como candidato y empieza una nueva eleccion
		case <-temporizador.C:
			nr.Logger.Println("Nodo %d: Ha vencido el timeout de eleccion y " +
...
func (nr *NodoRaft) protocoloLider() {
	...
	case <-nr.SoySeguidor:
		nr.Estado = SEGUIDOR
		nr.Logger.Println("Nodo %d: Era lider y ahora soy seguidor en el " +
				"mandato %d \n", nr.Yo, nr.CurrentTerm)
	case <-temporizadorLatidos.C:
		nr.Logger.Println("Nodo %d: Soy lider y sigo siendolo en el " +
				"mandato %d \n", nr.Yo, nr.CurrentTerm)
	}
}
...
func (nr *NodoRaft) enviarLatidosATodos() {
	nr.Logger.Println("Nodo %d: Soy lider y envio latidos en el " +
	"mandato %d \n", nr.Yo, nr.CurrentTerm)
	...
	for nodo:=0; nodo < len(nr.Nodos); nodo++ {
		if nodo != nr.Yo {
			...
			if len(nr.Log) - 1 >= nextIndexNodo_i {
				...
				entrada := EntradaLog{
					Operacion: nr.Log[nr.nextIndexNodo_i].Operacion,
					Term: nr.Log[nr.nextIndexNodo_i].Term,
...
func (nr *NodoRaft) envioDeLatido(nodo int, nextIndex int,
	...
	if args.Entrada == EntradaLog { // Enviado latido
